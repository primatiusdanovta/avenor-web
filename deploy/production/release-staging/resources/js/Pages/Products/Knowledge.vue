...
const normalizeWords = (value) => String(value || '')
    ...
    .replace(/\s+/g, ' ')
    .trim();
const detailDisplayLabel = (detail) => {
    const detailText = normalizeWords(detail.detail);
    const jenisText = normalizeWords(detail.jenis);

    return jenisText ? `${detailText} kategori ${jenisText}` : detailText;
};
</script>
