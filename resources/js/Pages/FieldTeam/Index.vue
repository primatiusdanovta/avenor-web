    ...
    <div class="row">
        <div class="col-12">
            <div class="card card-outline card-secondary">
                <div class="card-header"><h3 class="card-title">Filter KPI {{ entityLabelPlural }}</h3></div>
                <div class="card-body">
                    <form class="row align-items-end" @submit.prevent="submitSearch">
                        ...
                        <div class="col-md-4">
                            <label class="mb-1">Cari {{ entityLabel }}</label>
                            <div class="d-flex">
                                <input v-model="searchForm.search" type="text" class="form-control mr-2" :placeholder="`Cari ${entityLabel.toLowerCase()}`">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card card-outline card-primary">
                <div class="card-body role-switcher">
                    <div>
                        <div class="role-switcher__eyebrow">Mode Monitoring</div>
                        <h3 class="role-switcher__title mb-1">Field Team</h3>
                        <div class="text-muted">Pindahkan fokus admin antara marketing dan sales field executive tanpa keluar halaman.</div>
                    </div>
                    <div class="role-switcher__actions">
                        <Link
                            v-for="option in roleOptions"
                            :key="option.value"
                            :href="buildRoleSwitchHref(option.value)"
                            class="role-switcher__pill"
:class="{ 'role-switcher__pill--active': option.value === entityRole }"
                        >
                            <span>{{ option.label }}</span>
<small>{{ option.value === entityRole ? 'Role aktif' : 'Buka monitoring' }}</small>
                        </Link>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-12">
            <div class="card card-outline card-info">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">Daftar {{ entityLabelPlural }}</h3>
                </div>
                <div class="card-body p-0 table-responsive">
                    <table class="table table-hover mb-0">
                        <thead><tr><th>Nama</th><th>Status</th><th>Absensi</th><th>Lokasi Terakhir</th><th>Pergerakan</th><th>Total KPI</th><th>Penjualan</th><th>Kehadiran</th><th>Jam Kerja/Hari</th><th class="action-column">Aksi</th></tr></thead>
                        <tbody>
                            <tr v-for="item in fieldMembers" :key="item.id_user">
                                ...
                                <td>
                                    <div v-if="item.latest_location_summary" class="location-cell">
                                        <strong>{{ item.latest_location_summary.recorded_at }}</strong>
                                        <span>{{ item.latest_location_summary.source }}</span>
                                    </div>
                                    <span v-else class="text-muted">Belum ada lokasi</span>
                                </td>
                                <td>
                                    <span class="movement-chip" :class="item.movement_status.samples > 0 ? 'movement-chip--active' : 'movement-chip--idle'">
                                        {{ item.movement_status.label }}
                                    </span>
                                    <div class="small text-muted mt-1">{{ item.movement_status.recorded_at || 'Belum ada sampel' }}</div>
                                </td>
                                ...
                                <td>
                                    <div class="action-group">
                                        ...
                                        <button type="button" class="btn btn-xs btn-danger" @click="removeEntity(item)">
                            ...
                            <tr v-if="!fieldMembers.length"><td colspan="10" class="text-center text-muted py-4">Belum ada data {{ entityLabel.toLowerCase() }}.</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    <BootstrapModal :show="showCreateModal" :title="`Tambah ${entityLabel}`" size="lg" @close="closeCreateModal">
        <div class="crud-modal-body">
            <div class="form-group mb-0"><label>Nama {{ entityLabel }}</label><input v-model="createForm.nama" type="text" class="form-control"></div>
        ...
        <template #footer>
            ...
            <button type="button" class="btn btn-primary" :disabled="createForm.processing" @click="submitCreate">Tambah {{ entityLabel }}</button>
        </template>
    </BootstrapModal>

    <BootstrapModal :show="showEditModal" :title="`Edit ${entityLabel}`" size="lg" @close="closeEditModal">
        <div class="crud-modal-body">
            <div class="form-group mb-0"><label>Nama {{ entityLabel }}</label><input v-model="editForm.nama" type="text" class="form-control"></div>
    ...
    <BootstrapModal :show="showDetailModal" :title="detailModalTitle" size="xl" @close="closeDetailModal">
        <div v-if="detailLoading" class="text-center py-4 text-muted">Memuat detail {{ entityLabel.toLowerCase() }}...</div>
        ...
        <div v-else-if="selectedMember">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-3">
                <div>
                    <h4 class="mb-1">{{ selectedMember.nama }}</h4>
                    <div class="text-muted">Status akun: {{ selectedMember.status }}</div>
                </div>
                <div class="d-flex flex-wrap gap-2 mt-3 mt-lg-0">
                    <span class="badge badge-primary px-3 py-2">KPI {{ selectedMember.current_kpi.total_score.toFixed(2) }}</span>
                    <span class="badge policy-badge" :class="returnPolicyBadgeClass(selectedMember.require_return_before_checkout)">
                        {{ selectedMember.require_return_before_checkout ? 'Wajib Pengembalian' : 'Checkout Tanpa Pengembalian' }}
                    </span>
                </div>
            </div>

            <div class="row detail-grid mb-3">
                <div class="col-md-6 col-xl-3 mb-3"><div class="detail-card"><div class="detail-label">Product Terjual</div><div class="detail-value">{{ selectedMember.current_kpi.quantity_sold }}/{{ selectedMember.current_kpi.sales_target }} pcs</div></div></div>
                <div class="col-md-6 col-xl-3 mb-3"><div class="detail-card"><div class="detail-label">Kehadiran</div><div class="detail-value">{{ selectedMember.current_kpi.attendance_days }}/{{ selectedMember.current_kpi.attendance_target }} hari</div></div></div>
                <div class="col-md-6 col-xl-3 mb-3"><div class="detail-card"><div class="detail-label">Jam Kerja / Hari</div><div class="detail-value">{{ selectedMember.current_kpi.average_hours_per_day.toFixed(2) }}/{{ selectedMember.current_kpi.hours_target }} jam</div></div></div>
                <div class="col-md-6 col-xl-3 mb-3"><div class="detail-card"><div class="detail-label">Status Hari Ini</div><div class="detail-value">{{ selectedMember.today_attendance?.status || 'belum absen' }}</div></div></div>
            </div>

            <div class="card bg-light border-0 mb-3"><div class="card-body py-3"><div class="row"><div class="col-md-4 mb-3 mb-md-0"><div class="small text-muted">Check In</div><div class="font-weight-bold">{{ selectedMember.today_attendance?.check_in || '-' }}</div></div><div class="col-md-4 mb-3 mb-md-0"><div class="small text-muted">Check Out</div><div class="font-weight-bold">{{ selectedMember.today_attendance?.check_out || '-' }}</div></div><div class="col-md-4"><div class="small text-muted">Catatan</div><div class="font-weight-bold">{{ selectedMember.today_attendance?.notes || '-' }}</div></div></div></div></div>

            <div class="row">
                <div class="col-lg-7 mb-3 mb-lg-0">
                    <div class="section-panel h-100">
                        <div class="section-title">Ringkasan KPI</div>
                        <div class="summary-line"><span>Skor Penjualan</span><strong>{{ selectedMember.current_kpi.sales_score.toFixed(2) }} / 70</strong></div>
                        <div class="summary-line"><span>Skor Kehadiran</span><strong>{{ selectedMember.current_kpi.attendance_score.toFixed(2) }} / 20</strong></div>
                        <div class="summary-line"><span>Skor Jam Kerja</span><strong>{{ selectedMember.current_kpi.hours_score.toFixed(2) }} / 10</strong></div>
                        <div class="section-subtitle mt-4">Riwayat KPI 6 Bulan</div>
                        <div v-if="selectedMember.kpi_history?.length" class="history-stack">
                            <div v-for="history in selectedMember.kpi_history" :key="history.period_label" class="history-item">
                                <div>
                ...
                <div class="col-lg-5">
                    <div class="section-panel mb-3">
                        ...
                        <template v-if="selectedMember.latest_location">
                            <div class="small text-muted mb-2">{{ selectedMember.latest_location.latitude }}, {{ selectedMember.latest_location.longitude }}</div>
                            <div class="small text-muted mb-2">Direkam: {{ selectedMember.latest_location.recorded_at }}</div>
                            <div class="small text-muted mb-3">Sumber: {{ selectedMember.latest_location.source }}</div>
                            <iframe :src="selectedMember.latest_location.map_url" width="100%" height="220" style="border:0; border-radius: 12px;" loading="lazy"></iframe>
                        </template>
                        <div v-else class="text-muted">Belum ada data lokasi terbaru.</div>
                    </div>
                    <div v-if="selectedMember.bonus_summary" ref="bonusPanelRef" class="section-panel mb-3">
                        <div class="section-title">Bonus {{ selectedMember.bonus_summary.period_label }}</div>
                        <div class="summary-line"><span>Bonus Harian</span><strong>{{ selectedMember.bonus_summary.daily.bonus }}</strong></div>
                        <div class="summary-line"><span>Bonus Mingguan</span><strong>{{ selectedMember.bonus_summary.weekly.bonus }}</strong></div>
                        <div class="summary-line"><span>Bonus Bulanan</span><strong>{{ selectedMember.bonus_summary.monthly.bonus }}</strong></div>
                        <div class="summary-line"><span>Bonus Manual</span><strong>{{ selectedMember.bonus_summary.manual_bonus_total }}</strong></div>
                        <div class="summary-line"><span>Total Bonus</span><strong>{{ selectedMember.bonus_summary.bonus_total }}</strong></div>
                        <div v-if="selectedMember.bonus_summary.manual_bonus_entries?.length" class="mt-3">
                            <div class="small font-weight-bold mb-2">Riwayat Bonus Manual</div>
                            <div v-for="bonus in selectedMember.bonus_summary.manual_bonus_entries" :key="bonus.id" class="small text-muted mb-2">
                    ...
                    <div ref="carriedPanelRef" class="section-panel">
                        ...
                        <div v-if="selectedMember.carried_items?.length" class="carried-stack">
                            <div v-for="item in selectedMember.carried_items" :key="item.id_product_onhand" class="carried-item">
                        ...
                        <div v-else class="text-muted">Belum ada barang yang masih dibawa {{ entityLabel.toLowerCase() }}.</div>
                    </div>
                </div>
            </div>
        </div>
        <div v-else class="text-muted">Data {{ entityLabel.toLowerCase() }} belum tersedia.</div>
        <template #footer>
            <Link
                v-if="isSuperadmin && selectedMember"
                :href="adminUrl(`/product-onhands?user_id=${selectedMember.id_user}`)"
    ...
    <BootstrapModal :show="showMonitoringModal" :title="`Monitoring ${entityLabel}`" size="lg" @close="closeMonitoringModal">
        <div v-if="monitoringTarget">
            <h5 class="mb-2">{{ monitoringTarget.nama }}</h5>
            <p class="text-muted mb-3">Superadmin dapat mengatur apakah {{ entityLabel.toLowerCase() }} perlu mengembalikan barang. Jika perlu, {{ entityLabel.toLowerCase() }} harus mengembalikan barang sebelum checkout. Jika tidak, {{ entityLabel.toLowerCase() }} bisa checkout tanpa pengembalian.</p>
    ...
    <BootstrapModal :show="showDeleteModal" title="Konfirmasi Hapus" size="mobile-full" @close="closeDeleteModal">
        Hapus akun {{ entityLabel.toLowerCase() }} {{ deleteTarget?.nama }}?
        <template #footer>
...
const props = defineProps({
    ...
    entityLabel: { type: String, default: 'Marketing' },
    entityLabelPlural: { type: String, default: 'Marketing' },
    entityRole: { type: String, default: 'marketing' },
    roleOptions: { type: Array, default: () => [] },
});

const page = usePage();
const isSuperadmin = computed(() => page.props.auth?.user?.role === 'superadmin');
const selectedMember = ref(props.selectedMember ?? null);
const showCreateModal = ref(false);
const showEditModal = ref(false);
const showDetailModal = ref(Boolean(props.selectedMember));
...
const editForm = useForm({ id_user: props.selectedMember?.id_user ?? null, nama: props.selectedMember?.nama ?? '', status: props.selectedMember?.status ?? 'aktif', password: '', password_confirmation: '' });
...
watch(() => props.selectedMember, (marketing) => {
    if (!selectedMember.value) {
        selectedMember.value = marketing ?? null;
    }
...
const buildRoleSwitchHref = (role) => {
    const params = Object.entries(buildFilterParams({ role, selected: undefined }))
        .filter(([, value]) => value !== undefined && value !== null && value !== '')
        .reduce((carry, [key, value]) => ({ ...carry, [key]: String(value) }), {});

    return adminUrl('/field-team?' + new URLSearchParams(params).toString());
};

const detailModalTitle = computed(() => ({
    bonus: `Kelola Bonus ${props.entityLabel}`,
    carried: `Barang Dibawa ${props.entityLabel}`,
    detail: `Detail ${props.entityLabel}`,
}[detailModalContext.value] || `Detail ${props.entityLabel}`));
const submitSearch = () => router.get(adminUrl('/field-team'), buildFilterParams({ selected: undefined, role: props.entityRole }), { preserveScroll: true, preserveState: true, replace: true });
...
const submitCreate = () => createForm.post(adminUrl(`/field-team?role=${props.entityRole}`), { preserveScroll: true, onSuccess: () => closeCreateModal() });
...
const submitEdit = () => editForm.put(adminUrl(`/field-team/${editForm.id_user}?role=${props.entityRole}`), { preserveScroll: true, onSuccess: () => closeEditModal() });
const openDetail = async (id, options = {}) => {
    ...
    try {
        const query = new URLSearchParams({ month: String(searchForm.month), year: String(searchForm.year), role: props.entityRole });
        const { data } = await window.axios.get(adminUrl(`/field-team/${id}/detail?${query.toString()}`));
        ...
        detailError.value = error?.response?.data?.message || `Data ${props.entityLabel.toLowerCase()} belum tersedia.`;
    } finally {
...
const submitMonitoring = () => {
    ...
    monitoringForm.put(adminUrl(`/field-team/${monitoringTarget.value.id_user}/return-policy?role=${props.entityRole}`), {
        preserveScroll: true,
...
const submitManualBonus = () => {
    ...
    manualBonusForm.post(adminUrl(`/field-team/${selectedMember.value.id_user}/manual-bonuses?role=${props.entityRole}`), { preserveScroll: true });
};
...
const confirmDelete = () => {
    ...
    closeDeleteModal();
    router.delete(adminUrl(`/field-team/${id}?role=${props.entityRole}`), { preserveScroll: true });
};
...
.role-switcher__pill--active small {
...
@media (max-width: 991.98px) {
    ...
    .role-switcher__pill { min-width: 100%; }
}
