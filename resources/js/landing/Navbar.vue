        ...
        <div class="container">
            <div class="landing-navbar__inner">
                <nav class="landing-navbar__menu landing-navbar__menu--left" aria-label="Primary">
                    <a :href="homeHref" @click="handleNavClick($event, homeHref)">{{ uiLabels.home_label }}</a>
                    <a :href="collectionHref" @click="handleNavClick($event, collectionHref)">{{ uiLabels.collection_label }}</a>
                </nav>
                ...
                <nav class="landing-navbar__menu landing-navbar__menu--right" aria-label="Secondary">
                    <a :href="discoveryHref" @click="handleNavClick($event, discoveryHref)">{{ uiLabels.discovery_label }}</a>
                    <a :href="contactHref" @click="handleNavClick($event, contactHref)">{{ uiLabels.contact_label }}</a>
                </nav>
            </div>
        </div>
    </header>
...
const brandLabel = computed(() => props.uiLabels?.brand_label || props.uiLabels?.home_label || '');
